...
export function RecordingSettings({ onSave }: RecordingSettingsProps) {
  ...
  const handleRawSourcesToggle = async (enabled: boolean) => {
    const newPreferences = { ...preferences, save_raw_sources: enabled };
    setPreferences(newPreferences);
    await savePreferences(newPreferences);
  };
  ...
  const handleChangeFolder = async () => {
    try {
      ...
      // A dismissed dialog returns null and must leave the setting alone.
      if (!picked) return;
      ...
      toast.error('Could not change the recordings folder', {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Raw per-source capture — a diagnostic, priced accordingly */}
      <div className="flex items-center justify-between p-4 border rounded-lg">
        <div className="flex-1">
          <div className="font-medium">Keep raw audio from each source</div>
          <div className="text-sm text-muted-foreground">
            Saves the untouched microphone and system audio next to the recording, for
            diagnosing audio problems. Costs about 340 MB per hour per source.
          </div>
        </div>
        <Switch
          checked={preferences.save_raw_sources}
          onCheckedChange={handleRawSourcesToggle}
          disabled={saving}
        />
      </div>
      ...
      {!preferences.auto_save && (
        <div className="p-4 border rounded-lg bg-yellow-50 dark:bg-yellow-950/40">
