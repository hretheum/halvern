  ...
  return (
    <div
      className={cn(
        ...
        isAuthorized ? 'border-foreground bg-muted' : isDenied ? 'border-red-300 dark:border-red-800 bg-red-50 dark:bg-red-950/40' : 'bg-card border-border'
      )}
    >
      {/* Left side: Icon + Info */}
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div
          className={cn(
            ...
            isAuthorized ? 'bg-muted' : isDenied ? 'bg-red-100 dark:bg-red-900/40' : 'bg-muted'
          )}
        >
          <div className={cn(isAuthorized ? 'text-foreground' : isDenied ? 'text-red-500' : 'text-muted-foreground')}>{icon}</div>
        </div>
