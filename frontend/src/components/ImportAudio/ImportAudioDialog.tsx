...
export function ImportAudioDialog({
  ...
  // The remote endpoint option is filtered out: the import backend still
  // routes providers through its local whisper/parakeet switch, so offering
  // remote here would silently fall through to Whisper. Retranscription is
  // the remote-capable batch path.
  const {
  ...
  const availableModels = useMemo(
    () => allAvailableModels.filter((m) => m.provider !== 'remote'),
    [allAvailableModels]
  );
              ...
              {fileInfo ? (
                <div className="bg-muted rounded-lg p-4 space-y-3">
                  <div className="flex items-start gap-3">
                    <FileAudio className="h-8 w-8 text-blue-600 dark:text-blue-400 shrink-0" />
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground mt-1">
                        ...
                        <span className="text-blue-600 dark:text-blue-400 font-medium">{fileInfo.format}</span>
                      </div>
                    </div>
                  </div>
