...
// Settings tab value from app/settings/page.tsx's TABS array. The Transcription
// tab already renders Parakeet download progress inline (with a Cancel button
// the toast doesn't have), so the toast suppresses itself there to avoid
// showing the same percentage twice on screen at once.
const TRANSCRIPTION_SETTINGS_TAB = 'transcription';
...
function DownloadToastContent({
  ...
  return (
    <div className="flex items-center gap-3 w-full max-w-sm bg-card rounded-lg shadow-lg border border-border p-3 relative">
      ...
      {/* Icon */}
      <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isComplete ? 'bg-green-100 dark:bg-green-900/40' : hasError ? 'bg-red-100 dark:bg-red-900/40' : isCancelled ? 'bg-muted' : 'bg-muted'
        }`}>
        {isComplete ? (
...
export function useDownloadProgressToast() {
  const { activeSettingsTab } = useSidebar();
  ...
  const cleanupDownload = useCallback((modelName: string, delay: number = 4000) => {
    // Remove download from map after delay. Also dismiss the toast explicitly:
    // sonner's own per-call `duration` option is not reliable for closing a
    // `toast.custom` toast that was re-rendered via the same id (it was created
    // with duration: Infinity while downloading, then updated with a finite
    // duration on completion) — nothing else in this file ever calls
    // `toast.dismiss`, which is why finished downloads stayed on screen forever.
    setTimeout(() => {
  ...
  useEffect(() => {
    downloads.forEach((download) => {
      // The Transcription settings tab already renders Parakeet download
      // progress inline (with a Cancel button this toast doesn't have) — skip
      // the toast there so the same percentage isn't shown twice at once.
      if (download.source === 'parakeet' && activeSettingsTab === TRANSCRIPTION_SETTINGS_TAB) {
        toast.dismiss(`download-${download.modelName}`);
        return;
      }
  ...
  }, [downloads, dismissedModels, showDownloadToast, activeSettingsTab]);
