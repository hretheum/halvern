...
export function ModelManager({
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      ...
      const progressFn = await listen<{ modelName: string; progress: number }>(
      ...
      // Download complete
      const completeFn = await listen<{ modelName: string }>(
      ...
      // Download error
      const errorFn = await listen<{ modelName: string; error: string }>(
  ...
  if (error) {
    return (
      <div className={`bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-lg p-4 ${className}`}>
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      {/* User-provided models: discovery picks up any ggml-*.bin dropped into
          the models directory, so all the user needs is the convention. */}
      <p className="text-xs text-muted-foreground pt-1">
        You can also use your own Whisper model: place a <code>ggml-*.bin</code> file
        (e.g. a fine-tuned model) in the app&apos;s <code>models</code> folder and it
        will appear here after the model list refreshes.
      </p>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-blue-600 dark:text-blue-400">{Math.round(downloadProgress)}%</span>
              </div>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="text-xs text-muted-foreground hover:text-red-600 font-medium transition-colors px-2 py-1 rounded hover:bg-red-50 dark:hover:bg-red-950/60"
