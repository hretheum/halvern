...
// BlockNote is the heaviest dependency in the app — around 730 KB of the
// meeting-details route's JavaScript. Loading it on demand keeps it out of
// the route's initial bundle: the panel's toolbars, title, and generator
// controls render immediately, and the editor arrives right after.
//
// The imperative handle goes through the `innerRef` prop rather than `ref`
// because `next/dynamic` binds `ref` to its own retry object and forwards
// only props to the loaded component.
const BlockNoteSummaryView = dynamic(
  () =>
    import('@/components/AISummary/BlockNoteSummaryView').then(
      (mod) => mod.BlockNoteSummaryView
    ),
  {
    ssr: false,
    loading: () => (
...
export function SummaryPanel({
  ...
  return (
    <div className="flex-1 min-w-0 flex flex-col bg-card overflow-hidden">
      {/* Summary toolbar, per the mockup: selectors on the left, save state
          on the right, one slim row. Only shown when a summary exists. */}
      {aiSummary && !isSummaryLoading && (
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 px-5 py-2 border-b border-border">
      ...
      {isSummaryLoading ? (
        <div className="flex flex-col h-full">
          ...
          <div className="flex flex-wrap items-center justify-center min-w-0 gap-2 px-2 pt-8 pb-4">
            <SummaryGeneratorButtonGroup
              modelConfig={modelConfig}
              setModelConfig={setModelConfig}
              onSaveModelConfig={onSaveModelConfig}
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {summaryResponse && (
            <div className="fixed bottom-0 left-0 right-0 bg-card shadow-lg p-4 max-h-1/3 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-card p-4 rounded-lg shadow-xs mt-4">
          ...
          <div className="workshop-summary-doc px-7 py-6 w-full max-w-[720px]">
