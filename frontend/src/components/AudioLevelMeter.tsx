/**
 * Microphone level meters. Currently rendered by nothing.
 *
 * These were fed by a backend module that emitted `sin(counter) * 0.8` as the
 * level, so the meter moved convincingly for a muted microphone. That module
 * is gone. The components themselves are correct and stay: they are the
 * display half of a feature whose measuring half exists but is not wired —
 * see the module docs in `src-tauri/src/audio/level_monitor.rs` for what has
 * to be solved before this can be shown again.
 */
...
export function AudioLevelMeter({
  ...
  return (
    <div className={`flex items-center space-x-2 ${className}`}>
      ...
      <div className={`${sizes.text} text-muted-foreground font-mono min-w-12 text-right`}>
        {rmsPercent}%
      </div>
    </div>
  );
