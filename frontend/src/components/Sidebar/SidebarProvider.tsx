...
/**
 * App-level meeting context. The sidebar drawer this provider was born for
 * is gone; what remains is the state its consumers still share: the current
 * meeting, the lightweight meetings list some flows update optimistically,
 * the recording entry point, summary polling, and which settings category
 * is open (so globally-mounted toasts can avoid duplicating it).
 */
interface SidebarContextType {
  ...
  // Which Settings category is currently open (null when Settings isn't
  // mounted). Lets globally-mounted components (e.g. the download progress
  // toast) avoid duplicating a status a settings panel already shows inline.
  ...
  const handleRecordingToggle = () => {
    ...
    if (!isRecording) {
      if (pathname === '/record') {
        ...
        // Navigate to the recording screen and use the auto-start mechanism
        sessionStorage.setItem('autoStartRecording', 'true');
        router.push('/record');
      }

      Analytics.trackButtonClick('start_recording', 'library');
    }
    // The actual recording start/stop is handled on the recording screen
  };
