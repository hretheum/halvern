...
import { useState, useEffect, useCallback, useRef, useImperativeHandle, type RefObject } from 'react';
...
interface BlockNoteSummaryViewProps {
  ...
  /**
   * Imperative handle, passed as a normal prop rather than via `ref`.
   *
   * This component is loaded with `next/dynamic` (it pulls in BlockNote, by
   * far the heaviest dependency in the app). `next/dynamic` claims the `ref`
   * for itself — it binds it to its own `{ retry }` object and forwards only
   * `props` to the loaded component — so a real `ref` would silently resolve
   * to the wrong thing and break saving. Props are forwarded intact, hence
   * this one.
   */
  innerRef?: RefObject<BlockNoteSummaryViewRef | null>;
}
...
export function BlockNoteSummaryView({
  summaryData,
  onSave,
  onSummaryChange,
  status = 'idle',
  ...
  useEffect(() => {
    if (format === 'markdown' && data?.markdown && editor) {
      try {
        ...
        const blocks = editor.tryParseMarkdownToBlocks(data.markdown);
  ...
  // Expose methods to parent via the innerRef prop (see its doc comment)
