  ...
  return (
    <div 
      className={`group relative min-h-[24px] flex items-start rounded transition-all duration-150 ease-in-out
        ${isSelected ? 'bg-blue-50 dark:bg-blue-950/40 ring-1 ring-blue-200 shadow-xs' : 'hover:bg-accent'}`}
    ...
    >
      {block.type === 'bullet' && (
        <div className="shrink-0 mr-2 select-none mt-[2px]">•</div>
      )}
      ...
      <div className="relative flex-1 py-0.5 px-1">
        <textarea
          ...
          rows={1}
          className={`
            w-full resize-none overflow-hidden bg-transparent border-none p-0 focus:outline-hidden focus:ring-0
          ...
          >
            {filteredCommands.map((cmd, index) => (
              <button
                key={cmd.id}
                className={`
                  w-full text-left px-3 py-2 flex items-center space-x-3 hover:bg-accent
                  ${index === selectedCommandIndex ? 'bg-muted' : ''}
                `}
                onClick={() => handleCommandSelect(cmd)}
                onMouseEnter={() => setSelectedCommandIndex(index)}
              >
                <span className="shrink-0 w-6 h-6 flex items-center justify-center bg-muted rounded text-muted-foreground">
                  {cmd.icon}
                </span>
