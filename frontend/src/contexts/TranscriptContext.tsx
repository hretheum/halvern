...
import { speakerPrefix, normalizeSpeaker } from '@/lib/speaker-labels';
...
export function TranscriptProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const setupRecordingListeners = async () => {
      try {
        ...
        const startedFn = await recordingService.onRecordingStarted(async () => {
          try {
        ...
        unlistenRecordingStarted = startedFn;

        // Listen for recording-stopped event
        const stoppedFn = await recordingService.onRecordingStopped(async (payload) => {
          try {
            if (currentMeetingId) {
        ...
        unlistenRecordingStopped = stoppedFn;
      } catch (error) {
      ...
      if (recordingState.isRecording && transcripts.length === 0) {
        try {
          ...
          const formattedTranscripts: Transcript[] = history.map((segment: any) => ({
            ...
            // Rust: recording_saver::TranscriptSegment.source -> frontend: Transcript.speaker
            // normalizeSpeaker for symmetry with useTranscriptRecovery. Here we read the
            // state of the current process, so a legacy "Audio" value cannot reach us —
            // but two different filters over the same class of data invite the question
            // of which one somebody forgot to guard.
            speaker: normalizeSpeaker(segment.source),
          }));
  ...
  const copyTranscript = useCallback(() => {
    ...
    const fullTranscript = transcripts
      .map(t => `${formatTime(t.audio_start_time)} ${speakerPrefix(t.speaker)}${t.text}`)
      .join('\n');
