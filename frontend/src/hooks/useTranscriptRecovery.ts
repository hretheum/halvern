...
import { normalizeSpeaker } from '@/lib/speaker-labels';
  ...
  const recoverMeeting = useCallback(async (meetingId: string): Promise<{ success: boolean; audioRecoveryStatus?: AudioRecoveryStatus | null; meetingId?: string }> => {
    ...
    try {
      ...
      const formattedTranscripts = transcripts.map((t, index) => ({
        ...
        // StoredTranscript is a raw TranscriptUpdate written by object spread
        // (indexedDBService.ts), so the source field is called `source`, not `speaker`.
        // normalizeSpeaker rejects anything outside 'mic'/'system' — older entries from
        // when `source` was hardcoded to "Audio" would otherwise land in the `speaker`
        // column and surface as an "Audio" label in the UI, the export and the summary.
        speaker: normalizeSpeaker((t as any).source),
      }));
