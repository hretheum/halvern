...
export function useTranscriptionModels(transcriptModelConfig: TranscriptModelConfig | undefined) {
  ...
  const fetchModels = useCallback(async () => {
    ...
    // A configured remote endpoint shows up as one more model option.
    // Batch-only by design: this hook feeds the retranscription and import
    // dialogs, never the live-recording provider selection.
    try {
      const remoteConfig = await invoke<RemoteTranscriptionConfig | null>(
        'api_get_remote_transcription_config'
      );
      if (remoteConfig?.endpoint && remoteConfig?.model) {
        allModels.push({
          provider: 'remote' as const,
          name: remoteConfig.model,
          displayName: `🌐 Remote: ${remoteConfig.model}`,
          size_mb: 0,
        });
      }
    } catch (err) {
      console.error('Failed to fetch remote transcription config:', err);
    }
