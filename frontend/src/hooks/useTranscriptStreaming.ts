  ...
  useEffect(() => {
    ...
    return () => {
      if (streamingIntervalRef.current) {
        clearInterval(streamingIntervalRef.current);
        streamingIntervalRef.current = null;
        // The animation was interrupted mid-flight. Forget its segment, or the
        // re-run (strict mode, or a re-render mid-stream) sees a matching id,
        // skips the restart, and leaves the text frozen at its first few
        // characters. A completed animation keeps the id so re-renders with an
        // unchanged latest segment do not replay it.
        lastSegmentIdRef.current = null;
      }
    };
