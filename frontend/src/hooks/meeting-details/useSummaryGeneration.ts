...
import { speakerPrefix } from '@/lib/speaker-labels';
...
async function resolveSummaryLanguage(
  ...
  // The pinned global language outranks detection. Step 1 — the value stored
  // for this meeting — is the user's deliberate choice for this particular
  // meeting and always wins. But meetings created before the pin existed, or
  // ones whose write failed at creation, have no step 1 value; without this
  // step they would fall straight through to detection, ignoring the setting
  // visible in the UI. So the pin applies retroactively: it governs every
  // summary that has no choice of its own, not only new meetings. Detection
  // remains the behaviour of unpinned "Auto", which is what it was built for.
...
export function useSummaryGeneration({
  ...
  const buildSummaryTranscriptPayload = useCallback((allTranscripts: Transcript[]) => {
    ...
    return {
      // The speaker label in the text handed to the model — the summary's Owner
      // column is derived from it. Localized to match the template language.
