  ...
  useEffect(() => {
    ...
    const setupChunkDropListener = async () => {
      try {
        ...
        const fn = await listen<string>('chunk-drop-warning', (event) => {
  ...
  useEffect(() => {
    ...
    const setupTranscriptionErrorListener = async () => {
      try {
        ...
        const fn = await listen<{ error: string, userMessage: string, actionable: boolean }>('transcription-error', (event) => {
