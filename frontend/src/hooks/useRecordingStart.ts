...
export function useRecordingStart(
  ...
  // Check whether the configured transcription engine has a model ready. This
  // asks the provider the user is actually set up with rather than Parakeet
  // specifically, since onboarding installs Whisper for languages Parakeet
  // cannot hear and those users would otherwise be told to download a model
  // they already have.
  const checkTranscriptionReady = useCallback(async (): Promise<boolean> => {
    try {
      return await invoke<boolean>('transcription_model_available');
    } catch (error) {
  ...
  // Check if a model for the configured engine is currently downloading. This
  // decides between "wait a moment" and "you have nothing installed", so it has
  // to follow the same provider the readiness check does.
  const checkIfModelDownloading = useCallback(async (): Promise<boolean> => {
    try {
      return await invoke<boolean>('transcription_model_downloading');
    } catch (error) {
