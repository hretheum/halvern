...
class IndexedDBService {
  ...
  private readonly DB_NAME = 'HalvernRecoveryDB';
  private readonly DB_VERSION = 1;
  private initPromise: Promise<void> | null = null;

  /**
   * The store this one replaced, under the upstream project's name.
   *
   * Renaming a database is not an upgrade: `DB_VERSION` and `onupgradeneeded`
   * work within a single name, so the old store is simply never opened again.
   * That is the part worth caring about — `RecoveryPrompt` prunes on startup
   * (`deleteOldMeetings(7)`, `deleteSavedMeetings(24)`), and a store nothing
   * opens is a store nothing prunes. Left alone, it would keep meeting
   * transcripts on disk forever, unreachable and unmanaged, in a product whose
   * whole claim is that you decide what happens to your meetings.
   *
   * So it is deleted rather than migrated. Nothing in it can be more than
   * seven days old, because that is the ceiling this app already applies to
   * its own recovery data, and copying entries across would mean carrying a
   * one-time migration in the path that only runs after something has already
   * gone wrong.
   */
  private readonly SUPERSEDED_DB_NAME = 'MeetilyRecoveryDB';

  /**
   * Remove the pre-rename store, once, on the first init that finds it.
   *
   * Deliberately fire-and-forget and deliberately silent. This is cleanup, and
   * cleanup that cannot complete must never delay or fail the thing it is
   * cleaning up after — recovery has to work even if the old database is
   * locked by another window, already gone, or unreadable. A second run is
   * harmless: deleting a database that does not exist succeeds.
   */
  private dropSupersededDatabase(): void {
    try {
      const request = indexedDB.deleteDatabase(this.SUPERSEDED_DB_NAME);
      request.onsuccess = () => {
        console.info(`Removed the superseded recovery store (${this.SUPERSEDED_DB_NAME}).`);
      };
      request.onblocked = () => {
        // Another window still holds it open; the next launch will get it.
        console.debug(`${this.SUPERSEDED_DB_NAME} is open elsewhere; leaving it for next launch.`);
      };
      request.onerror = () => {
        console.debug(`Could not remove ${this.SUPERSEDED_DB_NAME}:`, request.error);
      };
    } catch (error) {
      console.debug(`Could not remove ${this.SUPERSEDED_DB_NAME}:`, error);
    }
  }
