...
export class Analytics {
  ...
  static async getPersistentUserId(): Promise<string> {
    try {
      ...
      let userId = sessionStorage.getItem('halvern_user_id');
      if (!userId) {
        userId = `user_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        sessionStorage.setItem('halvern_user_id', userId);
  ...
  /**
   * Reports that an error of a given type occurred.
   *
   * The message body is deliberately not sent. Callers pass raw errors
   * straight from the backend, and those carry full meeting folder paths,
   * remote endpoint URLs and device names — the same things the sanitizer
   * strips when they arrive under their own keys. `errorType` is the part
   * with diagnostic value and it is a fixed string chosen at the call site,
   * so it stays. The message is logged locally instead, where it is useful
   * and goes nowhere.
   *
   * The Rust sanitizer also drops `error_message`, so a caller that bypasses
   * this wrapper still cannot send one.
   */
  static async trackError(errorType: string, errorMessage: string): Promise<void> {
    console.error(`[Analytics] ${errorType}:`, errorMessage);
