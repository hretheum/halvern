...
impl WhisperEngine {
    ...
    pub async fn discover_models(&self) -> Result<Vec<ModelInfo>> {
        ...
        // User-provided models: any ggml-*.bin in the models directory that
        // the built-in catalog does not know. Dropping a file there is the
        // whole installation procedure — fine-tuned or community models work
        // without a catalog entry. Validated with the same GGML magic check
        // as catalog models; an invalid file is skipped (with a log line)
        // rather than surfaced as a broken entry.
        let catalog_filenames: std::collections::HashSet<&str> =
            model_configs.iter().map(|&(_, filename, ..)| filename).collect();
        if let Ok(entries) = std::fs::read_dir(models_dir) {
            for entry in entries.flatten() {
                ...
                let Some(stem) = filename.strip_prefix("ggml-").and_then(|s| s.strip_suffix(".bin"))
                else {
                    continue;
                };
                ...
                if self.validate_model_file(&path).await.is_err() {
                    log::warn!(
                        "Skipping user model file {} — not a valid GGML model",
                        filename
                    );
                    continue;
                }
                log::info!("Discovered user-provided Whisper model: {}", stem);
                models.push(ModelInfo {
                    name: stem.to_string(),
                    path: path.clone(),
                    size_mb: file_size_mb as u32,
                    accuracy: "Custom".to_string(),
                    speed: "Unknown".to_string(),
                    status: ModelStatus::Available,
                    description: "User-provided model file".to_string(),
                });
            }
        }
    ...
    fn calculate_repetition_ratio(text: &str) -> f32 {
        ...
        let repeated_words: usize = word_counts.values().map(|&count: &usize| count.saturating_sub(1)).sum();
