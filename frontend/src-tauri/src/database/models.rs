...
pub struct MeetingModel {
    ...
    /// How the recording started: `manual`, `auto` or `imported`. Absent for
    /// meetings recorded before the column existed.
    pub source: Option<String>,
    /// Application the audio came from, e.g. `Microsoft Teams`. Only
    /// auto-detected recordings can establish this.
...
pub struct Transcript {
    ...
    /// Where the utterance came from: `mic`, `system`, or absent for meetings predating
    /// the split of recognition by source.
    ///
    /// Without this field `SELECT *` silently drops the column: sqlx ignores result columns
    /// it has no home for, so a label written to the database never comes back out.
