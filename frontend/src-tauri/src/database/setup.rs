...
pub async fn initialize_database_on_startup(app: &AppHandle) -> Result<(), String> {
    ...
    if is_first_launch {
        ...
        // One-time catch-up for meetings that predate the language column;
        // in the background because it reads a metadata.json per meeting.
        let pool = db_manager.pool().clone();
        tauri::async_runtime::spawn(async move {
            crate::summary::commands::backfill_meeting_languages(&pool).await;
        });
