...
impl ParakeetEngine {
    ...
    pub async fn discover_models(&self) -> Result<Vec<ModelInfo>> {
        ...
        let model_configs = [
            ...
            // Full precision. Everything needed to run it already existed —
            // QuantizationType::FP32, the encoder-model.onnx file detection, the
            // download sizes and the loader branch — but the variant was missing
            // from this catalogue, so discovery never surfaced it and load_model
            // reported "not found" even with the weights on disk.
            ("parakeet-tdt-0.6b-v3-fp32", 2650, QuantizationType::FP32, "Precise (v3)", "Full precision, slower and much larger, noticeably better on degraded or band-limited audio"),
        ];
    ...
    async fn validate_model_directory(&self, model_dir: &Path) -> Result<()> {
        ...
        let expected_sizes: Vec<(&str, u64)> = if is_int8 {
            ...
            vec![
                // The graph file alone is ~41.8 MB; the weights that make this
                // model actually usable are in its .data sibling. Checking only
                // the graph file here is what let a 41 MB download pass as a
                // complete 2.44 GB model in the first place.
                ("encoder-model.onnx", 35_000_000),           // ~41.8 MB, min 35 MB
                ("encoder-model.onnx.data", 2_200_000_000),   // ~2.44 GB, min 2.2 GB
    ...
    pub async fn download_model_detailed(
        ...
        let base_url = if model_name.contains("-v2-") {
            ...
            //
            // This used to point at upstream's own server, which mirrors the
            // repository below. v3 is the engine chosen for all 25 European
            // languages, so the first run of most installations depended on a
            // host belonging to the project this one forked away from — and on
            // it staying up, staying free, and not minding the traffic.
            //
            // The files are the same ones: every name in both the fp32 and int8
            // sets resolves here, and the sizes match to the byte, the 2.4 GB
            // encoder weights included.
        ...
        let files_to_download = match model_info.quantization {
            ...
            QuantizationType::FP32 => vec![
                "encoder-model.onnx",
                // The fp32 encoder's weights exceed the 2 GB protobuf limit, so
                // upstream splits them into this external-data sibling (ONNX's
                // standard convention). Omitting it here — while the size table
                // below already accounted for it — was the whole bug: the loop
                // finished after 4 nominal files thinking it was done, having
                // fetched under 5% of the real total.
        ...
        let file_sizes: std::collections::HashMap<&str, u64> = match model_info.quantization {
            ...
            QuantizationType::FP32 => {
                // FP32 model sizes. The encoder is two files (see files_to_download
                // above); each needs its own entry here so the resume/skip-if-complete
                // check per file — which reads this map by exact filename — works for
                // both instead of only ever matching the graph file.
                [
                    ("encoder-model.onnx", 41_800_000u64),        // graph definition, ~41.8 MB
                    ("encoder-model.onnx.data", 2_440_000_000u64), // external weights, ~2.44 GB
