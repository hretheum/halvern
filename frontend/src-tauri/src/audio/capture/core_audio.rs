...
impl CoreAudioCapture {
    ...
    ///
    /// `process_object_ids` narrows the tap to those Core Audio processes, so a
    /// recording carries the meeting and nothing else the machine happens to be
    /// playing. An empty slice keeps the global tap, which is what a manually
    /// started recording gets: there is no application to point at, and a
    /// browser meeting cannot be separated from another tab in the same process
    /// anyway.
    pub fn new(process_object_ids: &[u32]) -> Result<Self> {
        ...
        // Mono either way: it is the more reliable shape for system capture on
        // macOS, and the pipeline treats this stream as mono downstream.
        let tap_desc = if process_object_ids.is_empty() {
            info!("🎙️ CoreAudio: Creating GLOBAL mono tap (everything the machine plays)");
            ca::TapDesc::with_mono_global_tap_excluding_processes(&cidre::ns::Array::new())
        } else {
            info!(
                "🎙️ CoreAudio: Creating SCOPED mono tap over {} process(es) {:?}",
                process_object_ids.len(),
                process_object_ids
            );
            let numbers: Vec<cidre::arc::R<cidre::ns::Number>> = process_object_ids
                .iter()
                .map(|id| cidre::ns::Number::with_u32(*id))
                .collect();
            ca::TapDesc::with_mono_mixdown_of_processes(&cidre::ns::Array::from_slice_retained(
                &numbers,
            ))
        };
        ...
        let agg_desc = cf::DictionaryOf::with_keys_values(
            ...
            &[
                ...
                cf::str!(c"halvern-audio-tap").as_type_ref(),
                &output_uid,
