...
/// Detection thresholds for the voice activity detector.
///
/// These used to be constants inside the constructor, which meant retuning them
/// required a recompile and left no record of what had been tried. They are
/// perceptual parameters that depend on the microphone, the room and — as a
/// Bluetooth headset in call mode demonstrates — on the codec in the path, so
/// they belong in data.
///
/// [`Default`] reproduces the historical hardcoded values exactly, so nothing
/// changes for existing callers.
#[derive(Debug, Clone, PartialEq)]
pub struct VadTuning {
    /// Confidence above which Silero declares speech. Lower catches more, at the
    /// cost of transcribing noise.
    pub positive_speech_threshold: f32,
    /// Confidence below which speech is considered ended.
    pub negative_speech_threshold: f32,
    /// How long a pause may last before a segment is closed. Longer values
    /// bridge natural pauses instead of fragmenting a sentence.
    pub redemption_time_ms: u32,
    /// Audio retained from before the onset, so word beginnings survive.
    pub pre_speech_pad_ms: u32,
    /// Audio retained after the offset.
    pub post_speech_pad_ms: u32,
    /// Segments shorter than this are dropped as fragments.
...
impl Default for VadTuning {
    fn default() -> Self {
        Self {
            ...
            // 400 ms is what `pipeline.rs:727` actually passes. The comment that
            // used to sit here claimed 2000 ms, which was never true of the live
            // path and made bench runs incomparable with the running app.
...
impl VadTuning {
    /// Thresholds read from Meetily Pro's `vad_config.json`.
    ///
    /// Kept as a named preset so the two can be compared with `vad-bench`
    /// rather than argued about. Pro is markedly more permissive, which is the
    /// leading explanation for it transcribing degraded Bluetooth audio that
    /// this build discards.
    pub fn pro_reference() -> Self {
        Self {
...
impl ContinuousVadProcessor {
    ...
    /// Builds a processor with explicit thresholds.
    pub fn with_tuning(input_sample_rate: u32, tuning: VadTuning) -> Result<Self> {
        ...
        config.positive_speech_threshold = tuning.positive_speech_threshold;
        config.negative_speech_threshold = tuning.negative_speech_threshold;
        config.redemption_time = Duration::from_millis(tuning.redemption_time_ms as u64);
        config.pre_speech_pad = Duration::from_millis(tuning.pre_speech_pad_ms as u64);
        config.post_speech_pad = Duration::from_millis(tuning.post_speech_pad_ms as u64);
        config.min_speech_time = Duration::from_millis(tuning.min_speech_time_ms as u64);

        debug!("Creating VAD session with: sample_rate={}Hz, tuning={:?}, input_rate={}Hz",
               VAD_SAMPLE_RATE, tuning, input_sample_rate);
