...
/// Appends `-2`, `-3`, ... to `base_name` until a name that does not already
/// exist under `base_path` is found.
///
/// The caller's timestamp has minute resolution, so two recordings started
/// under the same meeting name in the same minute would otherwise resolve to
/// the same folder and silently overwrite each other's metadata and
/// transcript as they run side by side.
fn unique_folder(base_path: &Path, base_name: &str) -> PathBuf {
    let candidate = base_path.join(base_name);
    if !candidate.exists() {
        return candidate;
    }
    for suffix in 2.. {
        let candidate = base_path.join(format!("{base_name}-{suffix}"));
        if !candidate.exists() {
            return candidate;
        }
    }
...
pub fn create_meeting_folder(
    ...
    let meeting_folder = unique_folder(base_path, &base_name);
...
/// Write mono f32 samples to an encoded file under `output_path`.
...
mod tests {
    ...
    #[test]
    fn sanitize_filename_neutralises_path_hostile_characters() {
        assert_eq!(sanitize_filename("Q3 / plans: review?"), "Q3 _ plans_ review_");
        assert_eq!(sanitize_filename("  padded  "), "padded");
    }

    #[test]
    fn unique_folder_returns_the_bare_name_when_nothing_is_there() {
        let root = TempDir::new().unwrap();
        assert_eq!(unique_folder(root.path(), "Standup"), root.path().join("Standup"));
    }

    #[test]
    fn unique_folder_appends_a_suffix_on_collision() {
        let root = TempDir::new().unwrap();
        std::fs::create_dir_all(root.path().join("Standup")).unwrap();

        assert_eq!(unique_folder(root.path(), "Standup"), root.path().join("Standup-2"));
    }

    #[test]
    fn unique_folder_skips_every_taken_suffix() {
        let root = TempDir::new().unwrap();
        for name in ["Standup", "Standup-2", "Standup-3"] {
            std::fs::create_dir_all(root.path().join(name)).unwrap();
        }

        assert_eq!(unique_folder(root.path(), "Standup"), root.path().join("Standup-4"));
    }

    #[test]
    fn same_meeting_name_in_the_same_minute_gets_distinct_folders() {
        // Regression test for the collision create_meeting_folder used to
        // have: two recordings started under one name inside the same
        // timestamp minute now land in separate folders instead of one
        // overwriting the other's metadata and transcript.
        let root = TempDir::new().unwrap();
        let base = root.path().to_path_buf();

        let first = create_meeting_folder(&base, "Collision", false).expect("first folder");
        let second = create_meeting_folder(&base, "Collision", false).expect("second folder");
    ...
    #[test]
    fn checkpoints_dir_only_appears_when_requested() {
        let root = TempDir::new().unwrap();
        let with = create_meeting_folder(&root.path().to_path_buf(), "With", true).unwrap();
        let without = create_meeting_folder(&root.path().to_path_buf(), "Without", false).unwrap();
