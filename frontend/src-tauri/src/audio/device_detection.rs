...
pub fn calculate_buffer_timeout(
    ...
    //
    // Doubling is integer arithmetic on the Duration; it used to go through
    // `mul_f32(2.0)`, which round-tripped the f64 above through single
    // precision and turned an exact 160ms into 159.999996ms. Harmless as a
    // timeout, but it made the value untestable against the millisecond figures
    // the rest of this module is specified in.
