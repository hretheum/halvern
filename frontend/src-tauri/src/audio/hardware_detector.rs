...
impl HardwareProfile {
    ...
    /// Split out of `has_windows_vulkan_runtime` so it can be tested anywhere:
    /// it takes the system root as an argument instead of reading `SystemRoot`,
    /// which is what lets the two tests below point it at a tempdir. The only
    /// production caller is Windows-only, so off Windows the sole callers are
    /// those tests — hence the conditional allow rather than a deletion.
