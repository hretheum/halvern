...
async fn create_meeting_with_transcripts(
    ...
    // Insert meeting. The source is known outright here — this path exists
    // only for files the user brought in — so it is written directly rather
    // than read back from metadata the way live recordings do it.
    sqlx::query(
    ...
    .bind(now)
    .bind(&folder_path)
    .bind(crate::audio::recording_saver::RecordingSource::Imported.as_str())
    .execute(&mut *tx)
    .await
