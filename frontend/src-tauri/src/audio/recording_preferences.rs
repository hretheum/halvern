...
pub async fn load_recording_preferences<R: Runtime>(
    ...
    let prefs = if let Some(value) = store.get("preferences") {
        match serde_json::from_value::<RecordingPreferences>(value.clone()) {
            // `mut` is load-bearing only on macOS, where the block below
            // overwrites the backend. Everywhere else the binding is never
            // written to, and an unqualified `mut p` warned on every build.
