...
pub(crate) fn create_transcript_segments(transcripts: &[(String, f64, f64)]) -> Vec<TranscriptSegment> {
    let without_sources: Vec<(String, f64, f64, Option<String>)> = transcripts
        .iter()
        .map(|(text, start, end)| (text.clone(), *start, *end, None))
        .collect();
    create_transcript_segments_with_sources(&without_sources)
}

/// Create transcript segments carrying an audio-source label per segment.
/// Each tuple is (text, start_ms, end_ms, source), where source uses the same
/// `mic` / `system` values the live pipeline stores in `transcripts.speaker`
/// (see `transcription::worker::source_label`); `None` means the source is
/// genuinely unknown — mixed-file retranscription, imports.
pub(crate) fn create_transcript_segments_with_sources(
...
pub(crate) fn write_transcripts_json(folder: &Path, segments: &[TranscriptSegment]) -> Result<()> {
    ...
    let json = serde_json::json!({
        ...
        "segments": segments.iter().enumerate().map(|(i, s)| {
            ...
            // Carry the source label when known, mirroring the live path's
            // per-segment `speaker`; older readers ignore the extra field.
            if let Some(speaker) = &s.speaker {
                entry["speaker"] = serde_json::json!(speaker);
            }
            entry
        }).collect::<Vec<_>>()
    });
