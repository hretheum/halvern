...
struct AudioMixerRingBuffer {
    ...
    /// Largest block emitted at once. No longer a fixed quantum: blocks are
    /// sized to the audio actually present, this only caps them.
    window_size_samples: usize,
    max_buffer_size: usize,  // Safety limit
    /// Smallest block worth emitting, so a trickle does not produce thousands
    /// of tiny mixes.
    min_emit_samples: usize,
    /// Whether each stream has ever delivered. An absent stream may be replaced
    /// with silence; a lagging one may not.
    mic_seen: bool,
    system_seen: bool,
}
...
impl AudioMixerRingBuffer {
    fn new(sample_rate: u32) -> Self {
        ...
        // 20 ms floor: short enough to keep latency low, long enough that a
        // device delivering in small bursts does not thrash the mixer.
        let min_emit_samples = (sample_rate as f32 * 0.020) as usize;
    ...
    /// True when a stream has delivered nothing while the other ran far ahead.
    ///
    /// Distinguishes "this stream does not exist" and "this stream died" from
    /// "this stream is a few milliseconds behind". Only the former two may be
    /// replaced with silence; padding a merely lagging stream is what used to
    /// punch holes in every recording.
    fn stalled(empty_side: usize, other_side: usize, window: usize) -> bool {
        empty_side == 0 && other_side >= window * 2
    }

    /// How many samples can be emitted while keeping both streams aligned.
    ///
    /// Capped at one window so a backlog is drained in steady chunks rather
    /// than one enormous block.
    fn available(&self) -> usize {
        let mic_len = self.mic_buffer.len();
        let sys_len = self.system_buffer.len();

        let mic_counts = self.mic_seen
            && !Self::stalled(mic_len, sys_len, self.window_size_samples);
        let sys_counts = self.system_seen
            && !Self::stalled(sys_len, mic_len, self.window_size_samples);

        let take = match (mic_counts, sys_counts) {
            // Both live: emit only what both can cover, so neither is invented.
            (true, true) => mic_len.min(sys_len),
            (true, false) => mic_len,
            (false, true) => sys_len,
            (false, false) => 0,
        };

        take.min(self.window_size_samples)
    }

    fn can_mix(&self) -> bool {
        self.available() >= self.min_emit_samples
    }

    /// Takes an equal number of samples from each stream.
    ///
    /// A stream that is absent or stalled contributes silence of exactly that
    /// length — correct, because there genuinely is no audio on that channel.
    /// A stream that is merely behind is never padded: the block is shortened
    /// instead, and the remainder is emitted once the samples arrive.
    fn extract_window(&mut self) -> Option<(Vec<f32>, Vec<f32>)> {
        let take = self.available();
        if take < self.min_emit_samples {
            return None;
        }

        let drain_or_silence = |buffer: &mut VecDeque<f32>| -> Vec<f32> {
            if buffer.len() >= take {
                buffer.drain(0..take).collect()
            } else {
                // Reachable only for an absent or stalled stream; `available`
                // guarantees a live stream has at least `take` samples.
                vec![0.0; take]
            }
        };

        let mic_window = drain_or_silence(&mut self.mic_buffer);
        let sys_window = drain_or_silence(&mut self.system_buffer);

        Some((mic_window, sys_window))
    }

    /// Drains whatever is left, for end-of-recording flush.
    ///
    /// Alignment no longer matters here: the goal is to lose nothing.
...
impl AudioCapture {
    ...
    pub fn process_audio_data(&self, data: &[f32]) {
        // Dropped while paused, not merely excluded from the duration count.
        // The system tap is global — it carries whatever the machine is
        // playing — so a paused recording that kept processing would capture
        // what the user put on after the meeting ended. Both sources share one
        // RecordingState, so they pause and resume together and stay in sync.
        ...
        // Record the source exactly as the device delivered it, before
        // resampling, filtering, noise suppression and mixing. Everything below
...
/// Shortest segment worth sending to the model: 50 ms at 16 kHz.
const MIN_SEGMENT_SAMPLES: usize = 800;

/// Builds the chunk handed to transcription from a single VAD segment.
///
/// Extracted because the same block appeared in both `run` and `flush_remaining_audio`;
/// once the sources were split there would have been four copies.
fn transcription_chunk_from(
...
/// Sends one speech segment, provided it is long enough.
fn emit_segment(
    segment: SpeechSegment,
    device_type: DeviceType,
    ...
    if segment.samples.len() < MIN_SEGMENT_SAMPLES {
        debug!(
            "⏭️ Skipping short segment {:?}: {:.1}ms ({} samples < {})",
            device_type, duration_ms, segment.samples.len(), MIN_SEGMENT_SAMPLES
        );
        return;
    }

    info!(
        "📤 VAD segment {:?}: {:.1}ms, {} samples",
        device_type, duration_ms, segment.samples.len()
    );

    let chunk = transcription_chunk_from(segment, device_type, *chunk_id_counter);
...
/// Runs one source's window through its own VAD and emits the speech it finds.
fn process_source(
    ...
    match vad.process_audio(window) {
        Ok(segments) => {
            for segment in segments {
                emit_segment(segment, device_type.clone(), sender, chunk_id_counter);
            }
        }
        // The message has to say which source failed — with two processors running,
        // a bare "VAD error" is useless.
        Err(e) => warn!("⚠️ VAD error for {:?}: {}", device_type, e),
    }
}

/// Flushes one VAD at the end of a recording and emits whatever it still held.
fn flush_source(
    vad: &mut ContinuousVadProcessor,
    device_type: DeviceType,
    sender: &mpsc::UnboundedSender<AudioChunk>,
    ...
    match vad.flush() {
        ...
        Err(e) => warn!("Failed to flush VAD for {:?}: {}", device_type, e),
    }
}

/// Whether the microphone can plausibly hear what the output device is playing.
///
/// Bluetooth means headphones in every practical case, so there is nothing to warn
/// about. Everything else — built-in speakers, a wired headset, an HDMI monitor —
/// is indistinguishable from the transport type alone: macOS reports a wired headset
/// on the built-in jack as `BUILT_IN` too.
///
/// The previous version matched device names against a keyword list. That only ever
/// worked for English and Polish; a German user gets "Lautsprecher" and a French one
/// "Haut-parleurs", so the check silently did nothing for them. Transport type is the
/// same in every language, and where it cannot decide, warning is the safer error:
...
///
/// Frozen at the value that ships, not the one that was intended. This used to
/// read `if cfg!(target_os = "macos") { 400 } else { 400 }`, under a comment
/// saying macOS Core Audio wants 900 ms and Windows 400 ms. Both arms were 400,
/// so the platform split never took effect and macOS has always run at 400 ms.
///
/// Whether 900 ms is actually better on macOS is untested here. Changing it
/// changes how transcripts are segmented, which is a behaviour change and
/// belongs in its own commit with its own evidence, not in a cleanup.
const REDEMPTION_TIME_MS: u32 = 400;

pub struct AudioPipeline {
    receiver: mpsc::UnboundedReceiver<AudioChunk>,
    transcription_sender: mpsc::UnboundedSender<AudioChunk>,
    // One VAD per source: the model never sees a mix, and every segment knows its origin.
...
impl AudioPipeline {
    pub fn new(
        ...
        if output_may_bleed_into_microphone(&system_device_name) {
            warn!(
                "⚠️ Output '{}' looks like built-in speakers rather than headphones. \
                 The microphone may pick up the other participants, so speaker labels can \
                 be wrong and passages may appear twice. Headphones avoid this entirely.",
                system_device_name
            );
        }
        ...
        // The VAD processor handles the 48 kHz → 16 kHz conversion itself. The
        // redemption time is how long it waits before calling a pause the end of
        // speech, which is what bridges a natural breath without fragmenting a
        // sentence into two segments.
        let make_vad = |label: &str| match ContinuousVadProcessor::new(sample_rate, REDEMPTION_TIME_MS) {
            Ok(processor) => processor,
            Err(e) => {
                error!("Failed to create VAD processor ({}): {}", label, e);
                panic!("VAD processor creation failed ({}): {}", label, e);
            }
        };

        let vad_mic = make_vad("mikrofon");
    ...
    pub async fn run(mut self) -> Result<()> {
        ...
        // Runs until the channel closes, deliberately not until recording stops.
        // This loop used to be `while self.state.is_recording()`, which exited the
        // moment stop_recording() was called and dropped the flush signal along with
        // every chunk still in flight — meetings completed early and lost their tail.
        //
        // The pipeline no longer holds `state` at all, so that condition cannot be
        // written back by accident. If you need it here, that is the bug returning:
        // the channel closing is what "recording finished" means on this side.
        loop {
                ...
                Ok(Some(chunk)) => {
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            // Step 3: each source goes to its own VAD. The transcript
                            // niesie autorstwo, a model nigdy nie widzi miksu. Mikser
                            // path below now only handles writing the file.
                            process_source(
                                &mut self.vad_mic,
                                &mic_window,
                                DeviceType::Microphone,
    ...
    fn flush_remaining_audio(&mut self) -> Result<()> {
        ...
        // The buffer tail goes straight to both VADs. The mixer is no longer needed
        // here: it used to mix this fragment purely to feed a single VAD, and the result
        // never reached the recording anyway.
        if let Some((mic_tail, sys_tail)) = self.ring_buffer.drain_remaining() {
            info!("🔚 Flushing {} samples left in the buffer", mic_tail.len());
            process_source(
                &mut self.vad_mic,
                &mic_tail,
                DeviceType::Microphone,
...
mod ring_buffer_tests {
    ...
    fn buffer() -> AudioMixerRingBuffer {
        AudioMixerRingBuffer::new(RATE)
    }

    /// Non-zero samples, so silence in the output can only come from padding.
    ...
    ///
    /// A Bluetooth headset in call mode delivers half the samples the pipeline
    /// expects. The mixer used to pad the shortfall with zeros, which put a
    /// ~340 ms hole into every 600 ms window — 40 to 56 percent of real
    /// recordings turned out to be digital silence, and transcription collapsed.
    #[test]
    fn a_lagging_stream_is_never_padded_with_silence() {
        let mut rb = buffer();
        let window = rb.window_size_samples;

        // System keeps up, microphone delivers half as fast.
        rb.add_samples(DeviceType::System, tone(window));
        rb.add_samples(DeviceType::Microphone, tone(window / 2));

        let (mic, sys) = rb.extract_window().expect("mixable");

        assert_eq!(mic.len(), sys.len(), "channels must come out the same length");
        assert_eq!(mic.len(), window / 2, "the block shrinks to what both channels cover");
        assert_eq!(longest_zero_run(&mic), 0, "missing samples must not be invented");
        assert_eq!(longest_zero_run(&sys), 0);

        // The system remainder is kept, not discarded.
        rb.add_samples(DeviceType::Microphone, tone(window / 2));
        let (mic2, _) = rb.extract_window().expect("the remainder should arrive");
        assert_eq!(mic2.len(), window / 2);
    }

    #[test]
    fn a_stream_that_never_appears_contributes_matching_silence() {
        let mut rb = buffer();
        let window = rb.window_size_samples;

        // Microphone only, as when no system audio device is available.
        rb.add_samples(DeviceType::Microphone, tone(window));

        let (mic, sys) = rb.extract_window().expect("mixable");
        assert_eq!(mic.len(), window);
        assert_eq!(sys.len(), window, "an absent channel is padded with silence of equal length");
        assert_eq!(longest_zero_run(&mic), 0);
        assert_eq!(sys.iter().all(|s| *s == 0.0), true);
    }

    #[test]
    fn a_stream_that_dies_mid_recording_does_not_block_the_other() {
        let mut rb = buffer();
        let window = rb.window_size_samples;

        // Both alive briefly, then the microphone stops delivering.
        rb.add_samples(DeviceType::Microphone, tone(window));
        rb.add_samples(DeviceType::System, tone(window));
        rb.extract_window().expect("pierwszy blok");

        // System races ahead on its own.
        rb.add_samples(DeviceType::System, tone(window * 2));

        let (_, sys) = rb
            .extract_window()
            .expect("a stalled channel must not block the recording forever");
        assert!(sys.iter().any(|s| *s != 0.0), "system audio must get through");
    }

    #[test]
    fn a_trickle_is_not_emitted_as_thousands_of_tiny_blocks() {
        let mut rb = buffer();

        // 5 ms, below the 20 ms floor.
        rb.add_samples(DeviceType::Microphone, tone(RATE as usize / 200));
        assert!(!rb.can_mix(), "not enough material to mix");
        assert!(rb.extract_window().is_none());
    }

    #[test]
    fn the_tail_survives_the_end_of_recording() {
        let mut rb = buffer();

        // Less than the floor, so the steady path will never emit it.
        rb.add_samples(DeviceType::Microphone, tone(100));
        rb.add_samples(DeviceType::System, tone(60));
        assert!(rb.extract_window().is_none());

        let (mic, sys) = rb.drain_remaining().expect("the tail must not be lost");
        assert_eq!(mic.len(), 100);
        assert_eq!(sys.len(), 100, "the shorter channel is padded to the longer one");
        assert!(rb.drain_remaining().is_none(), "a second call has nothing left");
    }

    #[test]
    fn a_backlog_is_drained_in_window_sized_blocks() {
        let mut rb = buffer();
        let window = rb.window_size_samples;

        rb.add_samples(DeviceType::Microphone, tone(window * 3));
        rb.add_samples(DeviceType::System, tone(window * 3));

        let (mic, _) = rb.extract_window().expect("mixable");
        assert_eq!(mic.len(), window, "one block at a time, not the whole backlog");
    }

    #[test]
    fn chunk_transkrypcji_zachowuje_zrodlo_i_przelicza_czas() {
        let segment = SpeechSegment {
            samples: vec![0.1; 1600],
            start_timestamp_ms: 2500.0,
            end_timestamp_ms: 3500.0,
            confidence: 0.9,
        };

        let chunk = transcription_chunk_from(segment, DeviceType::System, 7);

        // The source has to survive: before this change it was hardcoded to Microphone,
        // which is why the transcript could not carry attribution.
        assert_eq!(chunk.device_type, DeviceType::System);
        assert_eq!(chunk.chunk_id, 7);
        assert_eq!(chunk.sample_rate, 16000);
        // The VAD reports milliseconds; the transcription channel expects seconds.
        assert!((chunk.timestamp - 2.5).abs() < f64::EPSILON);
        assert_eq!(chunk.data.len(), 1600);
    }

    #[test]
    fn rozpoznaje_wbudowane_glosniki_jako_ryzyko_przesluchu() {
        assert!(output_may_bleed_into_microphone("MacBook Air Speakers"));
        // A Polish device name: the old keyword list was the only thing that made this
        // pass, and it failed for every other language. Transport type does not care.
        assert!(output_may_bleed_into_microphone("Głośniki MacBooka Air"));
        assert!(output_may_bleed_into_microphone("Lautsprecher des MacBook Air"));
        assert!(output_may_bleed_into_microphone("Realtek High Definition Audio"));

        // Headphones are safe: the microphone cannot hear the other party through them.
...
mod tests {
    ...
    /// Freezes the redemption time at what the code does today, not what the
    /// comment beside it used to promise.
    ///
    /// The value was written as `if cfg!(target_os = "macos") { 400 } else { 400 }`
    /// under a note saying macOS Core Audio wants 900 ms. Both arms were 400, so
    /// the platform split never took effect. This test exists so that discovering
    /// the discrepancy a second time does not turn into an unreviewed change to
    /// how transcripts are segmented.
    #[test]
    fn redemption_time_is_400ms_on_every_platform() {
        assert_eq!(REDEMPTION_TIME_MS, 400);
    }
}
