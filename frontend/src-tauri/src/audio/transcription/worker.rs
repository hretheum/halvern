...
/// The label stored in `transcripts.speaker`, matching the migration that created the column.
...
pub fn start_transcription_task<R: Runtime>(
    ...
    tokio::spawn(async move {
        ...
        // A single worker: ParakeetEngine::transcribe_audio takes an exclusive write lock
        // on the model (parakeet_engine.rs:461), so additional workers would only queue up
        // behind the same lock — more of them buys nothing.
                ...
                loop {
                    ...
                    match chunk {
                        Some(chunk) => {
                            ...
                            // Odczyt przed `transcribe_chunk_with_provider`, bo ta funkcja
                            // takes ownership of the chunk.
                            let chunk_source = source_label(&chunk.device_type).to_string();
                            ...
                            .await
                            {
                                Ok((transcript, confidence_opt, is_partial)) => {
                                    ...
                                    let meets_threshold = confidence_opt.is_none_or(|c| c >= confidence_threshold);
                                    ...
                                    if !transcript.trim().is_empty() && meets_threshold {
                                        ...
                                        // `sequence_id` is assigned here, at send time.
                                        ...
                                        if let Err(e) = app_clone.emit("transcript-update", &update) {
                                            error!("Worker {}: failed to send transcription result: {}", worker_id, e);
                                        }
