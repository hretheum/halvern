//! Real microphone level metering, from real capture streams. Not wired.
//!
//! `AudioLevelMonitor` is never constructed. What used to be wired instead was
//! `simple_level_monitor`, which emitted `counter.sin().abs() * 0.8` as the
//! microphone level, under a comment saying it existed to test the interface.
//! That module has been deleted: a meter that answers "is my microphone
//! working" with a sine wave answers "yes" to everyone, including people whose
//! microphone is muted.
//!
//! This module is the half that was supposed to replace it, and the reason it
//! did not is almost certainly `streams: Arc<Mutex<Vec<Stream>>>` below —
//! cpal's `Stream` is neither `Send` nor `Sync`, so that `Arc` cannot cross a
//! thread and clippy says so (`arc_with_non_send_sync`). Wiring this up means
//! solving that first: keep the streams on one thread and send samples over a
//! channel, rather than sharing the streams themselves.
//!
//! Until then there is no level meter in the interface, which is the honest
//! state. The display side survives in `frontend/src/components/AudioLevelMeter.tsx`.
