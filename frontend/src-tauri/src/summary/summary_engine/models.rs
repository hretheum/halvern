...
impl SamplingParams {
    ...
    /// Force greedy decoding when `HALVERN_BENCH_GREEDY` is set.
    ///
    /// The built-in models ship with different sampling presets — Qwen at
    /// temperature 0.5 with penalties, Gemma at 1.0 without — so any comparison
    /// between them measures the sampler as much as the model. The bake-off
    /// needs one arm with that difference removed
    /// (`docs/experiments/summary-model-bakeoff/01-design.md`).
    ///
    /// It has to live here because this is the only place local sampling is
    /// decided: `generate_with_builtin` takes no sampling arguments, so the
    /// `temperature` parameter threaded through `generate_meeting_summary`
    /// reaches the cloud providers and nothing else. That was discovered by
    /// running the bake-off and finding its "matched" arm had been running on
    /// shipped presets all along.
    ///
    /// Unset in every normal build, which is the whole of its safety.
    fn bench_greedy_override(&self, mut s: Self) -> Self {
        if std::env::var("HALVERN_BENCH_GREEDY").is_ok() {
            s.temperature = 0.0;
            s.top_k = 1;
            s.top_p = 1.0;
    ...
    pub fn sanitize_for_llama_helper(&self) -> Self {
        ...
        self.bench_greedy_override(Self {
            temperature,
            top_k,
            top_p,
            presence_penalty,
...
#[cfg(test)]
mod bench_override_tests {
    use super::*;

    #[test]
    fn greedy_override_is_off_unless_the_variable_is_set() {
        // The safety of this hook is entirely that it is unset in normal
        // builds, so that is what gets asserted. The environment is process
        // global and tests run in parallel, so this deliberately does not set
        // the variable — a test that did would flake every other test that
        // reads sampling.
        std::env::remove_var("HALVERN_BENCH_GREEDY");
        let qwen = SamplingParams::qwen35_summary(vec![]);
        let out = qwen.sanitize_for_llama_helper();
        assert_eq!(out.temperature, 0.5, "shipped preset must survive untouched");
