...
use crate::summary::metadata::{
    read_agenda_from_metadata, read_detected_summary_language_from_metadata,
    read_participants_from_metadata,
};
...
struct SummaryCacheSource {
    ...
    /// Fingerprint of the calendar agenda. `#[serde(default)]` so cache entries written
    /// before the agenda existed still parse: they fall back to an empty string, which
    /// no longer matches the new fingerprint, so the summary is regenerated. That is
    /// exactly what should happen — the prompt changed, so the result should too.
    #[serde(default)]
    agenda_fingerprint: String,
    /// Fingerprint of the invitation's participant list, defaulted for the same
    /// reason as the agenda: entries written before it existed still parse and
    /// stop matching, which regenerates a summary whose prompt has changed.
    #[serde(default)]
    participants_fingerprint: String,
    template_id: String,
...
fn build_summary_cache_source(
    ...
    SummaryCacheSource {
        ...
        agenda_fingerprint: stable_text_fingerprint(agenda.unwrap_or_default()),
        participants_fingerprint: stable_text_fingerprint(&participants.join("\n")),
...
impl SummaryService {
    ...
    /// Reads the calendar agenda stored beside the recording, if there is one.
    ///
    /// Mirrors `read_detected_summary_language`: the meeting row carries the
    /// folder path, and the recorder wrote the agenda into `metadata.json` there.
    ///
    /// Every failure returns `None` rather than propagating. A summary that
    /// succeeds without background context is a much better outcome than one
    /// that refuses to run because an invitation could not be read.
    async fn read_calendar_agenda(pool: &SqlitePool, meeting_id: &str) -> Option<String> {
        let meeting = match MeetingsRepository::get_meeting_metadata(pool, meeting_id).await {
            Ok(Some(meeting)) => meeting,
            Ok(None) => return None,
            Err(e) => {
                warn!(
                    "Failed to read meeting metadata for calendar agenda (meeting_id={}): {}",
                    meeting_id, e
                );
                return None;
            }
        ...
        match read_agenda_from_metadata(Path::new(&folder_path)) {
            Ok(agenda) => agenda,
            Err(e) => {
                warn!(
                    "Failed to read calendar agenda for meeting_id={}: {}",
                    meeting_id, e
                );
                None
            }
    ...
    /// Reads the invitation's participant list stored beside the recording.
    ///
    /// Mirrors `read_calendar_agenda` exactly, including its refusal to fail:
    /// a summary without the guest list is far better than no summary because
    /// an invitation could not be read.
    async fn read_calendar_participants(pool: &SqlitePool, meeting_id: &str) -> Vec<String> {
        let meeting = match MeetingsRepository::get_meeting_metadata(pool, meeting_id).await {
            Ok(Some(meeting)) => meeting,
            Ok(None) => return Vec::new(),
            Err(e) => {
                warn!(
                    "Failed to read meeting metadata for calendar participants (meeting_id={}): {}",
                    meeting_id, e
                );
                return Vec::new();
            }
        ...
        match read_participants_from_metadata(Path::new(&folder_path)) {
            Ok(participants) => participants,
            Err(e) => {
                warn!(
                    "Failed to read calendar participants for meeting_id={}: {}",
                    meeting_id, e
                );
                Vec::new()
            }
    ...
    pub async fn process_transcript_background<R: tauri::Runtime>(
        ...
        let agenda = Self::read_calendar_agenda(&pool, &meeting_id).await;
        if let Some(agenda) = &agenda {
            info!("📝 Calendar agenda available as context: {} chars", agenda.chars().count());
        }

        let participants = Self::read_calendar_participants(&pool, &meeting_id).await;
        if !participants.is_empty() {
            info!("📝 Calendar participants available as context: {}", participants.len());
        }
        ...
        match result {
            Ok((final_markdown, english_markdown, num_chunks)) => {
                ...
                .await
                {
                    Ok(true) => {
                        ...
                        // Obsidian auto-export. The summary is already persisted
                        // at this point, so nothing here may abort the flow:
                        // every failure is logged and swallowed.
                        Self::auto_export_to_obsidian(&pool, &meeting_id).await;
                    }
                    Ok(false) => {
                        // No summary_processes row existed for this meeting_id —
                        // the freshly generated summary was not written anywhere,
                        // so skip the export rather than send an empty one.
                        error!(
                            "No summary process row found while saving completed summary for {}",
                            meeting_id
                        );
                    }
                    Err(e) => {
            ...
            Err(e) => {
                ...
                if e.contains("cancelled") {
                    ...
                    match SummaryProcessesRepository::update_process_cancelled(&pool, &meeting_id).await {
                        Ok(false) => error!(
                            "No summary process row found while marking {} cancelled",
                            meeting_id
                        ),
                        Err(db_err) => error!(
    ...
    /// Writes the meeting into the configured Obsidian vault, when automatic
    /// export is enabled.
    ///
    /// Deliberately infallible from the caller's point of view. The summary has
    /// already been generated and stored by the time this runs, so a vault that
    /// is missing, read-only or on an unmounted volume must not turn a
    /// successful summary into a failed one.
    async fn auto_export_to_obsidian(pool: &SqlitePool, meeting_id: &str) {
        use crate::database::repositories::setting::SettingsRepository;

        let settings = match SettingsRepository::get_obsidian_settings(pool).await {
            Ok(s) => s,
            Err(e) => {
                error!("Could not read Obsidian export settings: {}", e);
                return;
            }
        };
        ...
        let Some(targets) = crate::export::targets_from_config(&settings) else {
            // Automatic export is on but no destination is configured.
            error!("Obsidian auto-export enabled but no destination configured");
            return;
        };

        match crate::export::export_meeting(pool, &targets, meeting_id).await {
            Ok(paths) => info!(
                "Obsidian export wrote {} file(s) for {}: {:?}",
                paths.len(),
                meeting_id,
                paths
            ),
            Err(e) => error!("Obsidian export failed for {}: {}", meeting_id, e),
        }
    }
    ...
    async fn update_process_failed(pool: &SqlitePool, meeting_id: &str, error_msg: &str) {
        ...
        match SummaryProcessesRepository::update_process_failed(pool, meeting_id, error_msg).await
        {
            Ok(false) => error!(
                "No summary process row found while marking {} failed",
                meeting_id
            ),
            Err(e) => error!(
...
mod tests {
    ...
    fn test_changed_summary_inputs_reject_cache() {
        ...
        let changed_sources = [
            // An agenda appearing where there was none changes the prompt, so a
            // summary generated without it must not be served from cache.
            build_summary_cache_source(
                "transcript body",
                "custom prompt",
                Some("Approve the Q3 budget"),
                &[],
                "standard_meeting",
                &template_fingerprint,
                3700,
            ...
            // Same for the guest list: names in the prompt change what the
            // model can attribute, so a summary made without them is stale.
            build_summary_cache_source(
                "transcript body",
                "custom prompt",
                None,
