...
/// The one place that turns a meeting folder's metadata.json into the value
/// `meetings.language` should hold. An explicit choice wins; `auto` is a
/// preference, not a language, so it falls through to whatever was detected.
fn resolve_language_from_metadata(folder: &std::path::Path) -> Option<String> {
...
/// Fill `meetings.language` for rows that predate the column.
///
/// Runs in the background on every startup but touches only rows where the
/// column is still NULL, so repeated runs do no work and can never overwrite
/// a value the normal write path has produced since. A meeting whose folder
/// is gone or whose metadata.json predates language tracking stays NULL —
/// an honest "unknown", not a guess.
pub async fn backfill_meeting_languages(pool: &sqlx::SqlitePool) {
    let rows: Vec<(String, String)> = match sqlx::query_as(
        "SELECT id, folder_path FROM meetings WHERE language IS NULL AND folder_path IS NOT NULL",
    )
    .fetch_all(pool)
    .await
    {
        Ok(rows) => rows,
        Err(e) => {
            log_warn!("Language backfill skipped - could not list meetings: {}", e);
            return;
        }
    };
    if rows.is_empty() {
    ...
    for (id, folder) in &rows {
        let Some(language) = resolve_language_from_metadata(std::path::Path::new(folder)) else {
            continue;
        };
        // The NULL guard re-checks per row: a summary generated while this
        // loop runs must not have its freshly written language clobbered.
        match sqlx::query("UPDATE meetings SET language = ? WHERE id = ? AND language IS NULL")
            .bind(&language)
            .bind(id)
            .execute(pool)
            .await
        {
            Ok(_) => filled += 1,
            Err(e) => log_warn!("Language backfill failed for meeting {}: {}", id, e),
        }
    }
    log_info!(
        "Language backfill: filled {} of {} meetings that were missing a language",
        filled,
        rows.len()
    );
}

/// Mirror a meeting's language from metadata.json into `meetings.language`.
///
/// The metadata file stays the source of truth — it travels with the recording
/// folder and outlives any database. The column exists so the library can
/// filter by language without opening a file per row.
///
/// Both writers call this, and both recompute from the file rather than from
/// the value they just wrote, so the order of an explicit choice and an
/// automatic detection cannot leave the column disagreeing with the file.
async fn sync_meeting_language_column(
    pool: &sqlx::SqlitePool,
    meeting_id: &str,
    folder: &std::path::Path,
) {
    let language = resolve_language_from_metadata(folder);

    if let Err(e) = sqlx::query("UPDATE meetings SET language = ? WHERE id = ?")
        .bind(language)
        .bind(meeting_id)
        .execute(pool)
        .await
    {
        log_warn!("Could not mirror language onto meeting {}: {}", meeting_id, e);
    }
}
    ...
    match resolve_meeting_folder(state.db_manager.pool(), &meeting_id).await? {
        MeetingFolderResolution::Folder(folder) => {
            write_summary_language_to_metadata(&folder, summary_language.as_deref())
                .map_err(|e| e.to_string())?;
            sync_meeting_language_column(state.db_manager.pool(), &meeting_id, &folder).await;
            read_summary_language_from_metadata(&folder)
                .map(MeetingSummaryLanguagePreference::metadata)
                .map_err(|e| e.to_string())
        }
...
pub async fn api_cancel_summary<R: Runtime>(
    ...
    if cancelled {
        ...
        match SummaryProcessesRepository::update_process_cancelled(pool, &meeting_id).await {
            Ok(false) => {
                log_error!("No summary process row found while cancelling {}", meeting_id);
                return Err("No summary process was recorded for this meeting".into());
            }
            Err(e) => {
