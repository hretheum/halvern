//! Embedded default templates, included at compile time.
//!
//! These are bundled into the binary and serve as fallbacks when custom
...
mod tests {
    ...
    fn test_builtin_templates_valid_json() {
        for id in list_builtin_template_ids() {
            let content = get_builtin_template(id).expect("listed id must resolve");
