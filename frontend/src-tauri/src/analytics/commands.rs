...
/// Telemetry credentials, supplied at build time.
///
/// This fork inherited the upstream project's hardcoded PostHog key, which
/// meant that anyone who turned the analytics switch on sent their usage to
/// the upstream author's project rather than to whoever built the app. Both
/// values are build-time environment variables now, and both are absent by
/// default: an unconfigured build creates no client and sends nothing, so
/// the switch is inert rather than pointed somewhere unexpected.
const TELEMETRY_API_KEY: Option<&str> = option_env!("HALVERN_TELEMETRY_API_KEY");
const TELEMETRY_ENDPOINT: Option<&str> = option_env!("HALVERN_TELEMETRY_ENDPOINT");

#[command]
pub async fn init_analytics() -> Result<(), String> {
    let Some(api_key) = TELEMETRY_API_KEY.filter(|key| !key.is_empty()) else {
        log::info!("Telemetry not configured for this build; analytics stays off");
        return Ok(());
    };

    let config = AnalyticsConfig {
        api_key: api_key.to_string(),
        host: TELEMETRY_ENDPOINT.map(|endpoint| endpoint.to_string()),
        enabled: true,
    };
