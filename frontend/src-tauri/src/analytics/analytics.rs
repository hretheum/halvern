...
const SENSITIVE_ANALYTICS_KEYS: &[&str] = &[
    ...
    "device_name",
    "user_agent",
    // Error bodies are free-form text assembled by whichever layer failed,
    // and in this app they routinely carry the very things the keys above
    // exist to keep out: "No audio file found in: <full meeting folder>",
    // remote endpoint URLs, device names. The error *type* accompanying them
    // is what carries the diagnostic value, and it survives.
    "error_message",
    "errorMessage",
];
...
impl AnalyticsClient {
    pub async fn new(config: AnalyticsConfig) -> Self {
        let client = if config.enabled && !config.api_key.is_empty() {
            // `posthog_rs::client(&str)` would quietly use the crate's own
            // default endpoint (PostHog Cloud US), which is how this config's
            // `host` field managed to look meaningful while doing nothing.
            // Building the options explicitly is what actually honours it.
            let mut options = posthog_rs::ClientOptionsBuilder::default();
            options.api_key(config.api_key.clone());
            if let Some(endpoint) = config.host.as_ref().filter(|h| !h.is_empty()) {
                options.api_endpoint(endpoint.clone());
            }
            match options.build() {
                Ok(options) => Some(Arc::new(posthog_rs::client(options).await)),
                Err(e) => {
                    log::error!("Invalid telemetry client options, analytics disabled: {}", e);
                    None
                }
            }
        } else {
