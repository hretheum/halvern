...
use crate::{
    database::{
        models::MeetingModel,
        repositories::{
            meeting::{MeetingQuery, MeetingSortField, MeetingsRepository},
...
/// One row of the meetings list, as the UI needs it.
///
/// `Meeting` above carries only id and title, which is why the list could
/// never group by date or sort by anything — the data was dropped here, not
/// missing from the database. This type is what `api_query_meetings` returns.
#[derive(Debug, Serialize, Deserialize)]
pub struct MeetingListItem {
    ...
    /// `manual`, `auto` or `imported`; absent for older meetings.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Excerpt around the search hit, present only when searching.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub snippet: Option<String>,
}

/// A page of the meetings list plus the total number of matches, so the UI can
/// page without a second round trip to learn how much there is.
#[derive(Debug, Serialize, Deserialize)]
...
pub struct MeetingTranscript {
    ...
    /// Where the utterance came from: `mic`, `system`, or absent for older meetings.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct MeetingMetadata {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub folder_path: Option<String>,
    /// Wall-clock length of the recording, when known.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub duration_seconds: Option<f64>,
    /// `manual`, `auto` or `imported`; absent for older meetings.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct TranscriptSegment {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub duration: Option<f64>,
    /// Audio source: `mic`, `system`, or absent for older meetings.
...
/// Query the meetings list with search, filters, sorting and paging.
///
/// Replaces the pattern of fetching every meeting and filtering on the client.
/// Text search runs through the FTS5 index over transcripts and also matches
/// titles; the excerpt around each hit is produced by SQLite rather than by
/// shipping whole transcripts across the bridge.
///
/// All arguments are optional. Defaults: newest first, 50 per page.
#[tauri::command]
#[allow(clippy::too_many_arguments)]
pub async fn api_query_meetings<R: Runtime>(
    _app: AppHandle<R>,
    state: tauri::State<'_, AppState>,
    ...
    limit: Option<i64>,
    offset: Option<i64>,
) -> Result<MeetingQueryResult, String> {
    // An unrecognised sort field falls back to date rather than erroring: the
    // list staying usable matters more than rejecting a stale client.
    let sort_field = match sort.as_deref() {
        Some("title") => MeetingSortField::Title,
        Some("duration") => MeetingSortField::Duration,
        Some("segments") => MeetingSortField::SegmentCount,
        Some("date") | None => MeetingSortField::CreatedAt,
        Some(other) => {
            log_warn!("Unknown sort field '{}', falling back to date", other);
            MeetingSortField::CreatedAt
        }
    };
    ...
    let pool = state.db_manager.pool();
    match MeetingsRepository::query_meetings(pool, &query).await {
        Ok((rows, total)) => {
            log_info!(
                "api_query_meetings returned {} of {} meetings",
                rows.len(),
                total
            );
            let items = rows
                ...
                .collect();
            Ok(MeetingQueryResult { items, total })
        }
        Err(e) => {
            log_error!("Error querying meetings: {}", e);
            Err(e.to_string())
        }
    }
}
...
pub async fn open_meeting_folder<R: Runtime>(
    ...
    // Get meeting with folder_path. The column list must cover every
    // MeetingModel field - FromRow fails at runtime on any missing column.
    let meeting: Option<MeetingModel> = sqlx::query_as(
        "SELECT id, title, created_at, updated_at, folder_path, duration_seconds, source, app_name, language FROM meetings WHERE id = ?",
    )
    .bind(&meeting_id)
    .fetch_optional(pool)
    .await
...
/// Saves the remote transcription endpoint configuration (batch jobs only).
/// An empty API key is stored as absent: self-hosted servers commonly run
/// without authentication and that is a supported configuration.
#[tauri::command]
pub async fn api_save_remote_transcription_config<R: Runtime>(
    ...
    let config = crate::audio::transcription::RemoteTranscriptionConfig {
        endpoint: endpoint.trim().to_string(),
        model: model.trim().to_string(),
        api_key: api_key.filter(|k| !k.trim().is_empty()),
    };
    ...
    match SettingsRepository::save_remote_transcription_config(pool, &config).await {
        Ok(()) => {
            log_info!("✅ Saved remote transcription config for endpoint: {}", config.endpoint);
            Ok(serde_json::json!({ "status": "success" }))
        }
        Err(e) => {
            log_error!("❌ Failed to save remote transcription config: {}", e);
            Err(format!("Failed to save remote transcription configuration: {}", e))
        }
    }
}

/// Gets the remote transcription endpoint configuration, or null when the
/// feature was never configured.
#[tauri::command]
pub async fn api_get_remote_transcription_config<R: Runtime>(
    _app: AppHandle<R>,
    state: tauri::State<'_, AppState>,
) -> Result<Option<crate::audio::transcription::RemoteTranscriptionConfig>, String> {
    let pool = state.db_manager.pool();
    SettingsRepository::get_remote_transcription_config(pool)
        .await
        .map_err(|e| {
            log_error!("❌ Failed to get remote transcription config: {}", e);
            format!("Failed to get remote transcription configuration: {}", e)
        })
}

/// Removes the remote transcription endpoint configuration entirely.
#[tauri::command]
pub async fn api_clear_remote_transcription_config<R: Runtime>(
    _app: AppHandle<R>,
    state: tauri::State<'_, AppState>,
) -> Result<serde_json::Value, String> {
    let pool = state.db_manager.pool();
    match SettingsRepository::clear_remote_transcription_config(pool).await {
        Ok(()) => {
            log_info!("✅ Cleared remote transcription config");
            Ok(serde_json::json!({ "status": "success" }))
        }
        Err(e) => {
            log_error!("❌ Failed to clear remote transcription config: {}", e);
            Err(format!("Failed to clear remote transcription configuration: {}", e))
        }
    }
}
