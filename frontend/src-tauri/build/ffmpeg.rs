...
pub fn ensure_ffmpeg_binary() {
    ...
    note!("Checking FFmpeg binary for target: {}", target);
    ...
    if binary_path.exists() {
        note!("Found cached FFmpeg binary: {}", binary_name);
        if verify_ffmpeg_binary(&binary_path) {
            note!("FFmpeg binary already cached and verified: {}", binary_name);
            return;
        } else {
            warn_build!("cached FFmpeg binary failed verification, re-downloading");
            let _ = std::fs::remove_file(&binary_path);
        }
    }

    note!("FFmpeg binary not found, downloading for {}", target);
    ...
    match download_and_extract_ffmpeg(&target, &binary_path) {
        Ok(()) => {
            note!("FFmpeg binary downloaded successfully: {}", binary_name);
...
fn download_and_extract_ffmpeg(
    ...
    note!("Fetching FFmpeg download URL for {}", target);

    // Get platform-specific download URL
    let url = get_ffmpeg_url_for_target(target)?;

    note!("Downloading from: {}", url);
    ...
    note!("Download size: {:.1} MB", total_size as f64 / 1_048_576.0);
    ...
    note!("Downloaded to: {:?}", archive_path);
    note!("Extracting FFmpeg binary...");
...
fn extract_ffmpeg_from_archive(
    ...
    note!("Found FFmpeg at: {:?}", ffmpeg_binary);
    ...
    for i in 0..archive.len() {
        ...
        let outpath = match file.enclosed_name() {
            ...
            None => {
                ...
                warn_build!("skipped ZIP entry with a path outside the extraction directory: {}", file.name());
                continue;
            }
        };
        ...
        Ok(output) => {
            if output.status.success() {
                ...
                if let Some(version_line) = stdout.lines().next() {
                    note!("FFmpeg verification passed: {}", version_line);
                }
                true
            } else {
                false
